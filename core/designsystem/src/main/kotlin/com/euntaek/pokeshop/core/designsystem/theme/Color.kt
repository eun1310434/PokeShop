package com.euntaek.pokeshop.core.designsystem.theme
...
object ColorSchemeDefault {
    const val DEFAULT_COLOR_OPACITY = 0.38f
}

@Composable
fun Color.addOpacity(opacity: Float = ColorSchemeDefault.DEFAULT_COLOR_OPACITY) =
    this.copy(alpha = opacity)


@Composable
fun Palette.toBrush(defaultColor: Color) = Brush.horizontalGradient(
    listOf(
        this.darkMutedSwatch?.let { Color(it.rgb) } ?: defaultColor,
        this.lightMutedSwatch?.let { Color(it.rgb) } ?: defaultColor,
        this.mutedSwatch?.let { Color(it.rgb) } ?: defaultColor,
        this.vibrantSwatch?.let { Color(it.rgb) } ?: defaultColor,
        this.darkVibrantSwatch?.let { Color(it.rgb) } ?: defaultColor,
        this.lightVibrantSwatch?.let { Color(it.rgb) } ?: defaultColor,
